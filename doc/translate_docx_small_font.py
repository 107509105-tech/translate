    ...
    for textbox in textboxes:
        ...
        for run in runs:
            ...
            if text_elem is not None and text_elem.text and text_elem.text.strip():
                ...
                sz = rPr.find(qn('w:sz'))
                if sz is None:
                    sz = run.makeelement(qn('w:sz'))
                    rPr.append(sz)
                sz.set(qn('w:val'), '16')  # 8pt = 16 半點

                # 同時設置 szCs (Complex Script 字體大小)
                szCs = rPr.find(qn('w:szCs'))
                if szCs is None:
                    szCs = run.makeelement(qn('w:szCs'))
                    rPr.append(szCs)
                szCs.set(qn('w:val'), '16')  # 8pt = 16 半點
