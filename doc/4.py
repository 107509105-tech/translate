...
class TranslationConfig:
    ...
    # Translator Settings (Google Translator via deep-translator)
    # Using traditional Chinese (zh-TW) to English translation
...
class TextDetector:
    ...
    @staticmethod
    def has_bracket_format(text):
        """Check if text has 【】bracket format (should remain independent paragraph)

        Returns True for independent item formats:
        - 【電源開關】：開啟及關閉電源。
        - 【a. Worksheet】 列印已完成之分析方法及結果。

        Returns False for sentence continuations:
        - 【Read】後開始進行... (【】followed by particle)
        """
        stripped = text.strip()
        ...
        after_bracket = match.group(1)

        # If followed by colon, it's an independent item
        if after_bracket.lstrip().startswith('：') or after_bracket.lstrip().startswith(':'):
            return True

        # If followed by Chinese particle (indicating continuation), it's NOT independent
        # Common particles: 後(after), 中(in/during), 內(within), 外(outside), 前(before), 等(etc)
        if after_bracket and after_bracket[0] in '後中內外前左右上下之的等至':
            return False

        # Check if bracket contains list marker (a. b. 1. 2. etc) - these are independent items
        bracket_content = re.match(r'^【([^】]+)】', stripped).group(1)
        if re.match(r'^[a-zA-Z0-9]+\.', bracket_content.strip()):
            return True

        # If followed by spaces (2+ spaces or tabs), likely an independent item
        if re.match(r'^[\s\u3000]{2,}', after_bracket):
            return True

        # Default: if it's a simple 【noun】 format at start, treat as independent
        # unless followed by particle (already handled above)
        return len(after_bracket.strip()) > 0
...
class TranslationEngine:
    """Handles translation logic using LLM and dictionaries"""

    def __init__(self, state: DocumentState):
        self.state = state

    def translate_text(self, text):
        """Main translation function with dictionary lookup and LLM fallback"""
...
class ParagraphProcessor:
    ...
    def record_long_space_paragraph(self, paragraph, para_index=None):
        ...
        # Check if paragraph has 【】bracket format - these should remain independent
        paragraph_text = paragraph.text.strip()
        has_bracket_format = TextDetector.has_bracket_format(paragraph_text)

        # If paragraph has no leading spaces OR has bracket format, end current group
        if not has_abnormal or has_bracket_format:
...
class BilingualTranslator:
    ...
    def translate_table(self, table):
        ...
        # FIRST PASS: Collect all unique cells to translate BEFORE any modification
        # This prevents issues with cell._element IDs changing during translation
        cells_to_translate = []
        seen_cell_elements = set()

        for row in table.rows:
            for cell in row.cells:
                cell_elem_id = id(cell._element)

                # Skip if already seen this cell element
                if cell_elem_id in seen_cell_elements:
                    continue
                seen_cell_elements.add(cell_elem_id)

                # Check if cell has Chinese paragraphs
        ...
        # SECOND PASS: Translate collected cells
        ...
        for section in doc.sections:
            ...
            for hf in headers + footers:
                ...
                for table in hf.tables:
                    ...
                    for row in table.rows:
                        for cell in row.cells:
                            # Skip if this cell was already processed (merged cells)
...
class FlowchartHandler:
    ...
    def add_english_paragraph_to_textbox(self, textbox, english_text, font_size_halfpts=11):
        """Add English paragraph below existing content in textbox"""
        ...
        # Create run with English text
        new_r = OxmlElement('w:r')

        # Set run properties (font, size)
        rPr = OxmlElement('w:rPr')

        # Font
        rFonts = OxmlElement('w:rFonts')
        rFonts.set(qn('w:ascii'), TranslationConfig.DEFAULT_FONT)
        rFonts.set(qn('w:hAnsi'), TranslationConfig.DEFAULT_FONT)
        rFonts.set(qn('w:cs'), TranslationConfig.DEFAULT_FONT)
        rPr.append(rFonts)

        # Font size
        sz = OxmlElement('w:sz')
        sz.set(qn('w:val'), str(font_size_halfpts))
        rPr.append(sz)
    ...
    def clone_and_translate_flowchart(self, table):
        ...
        # Translate cloned table (English-only mode for cloned flowchart)
        for row in translated_table.rows:
            for cell in row.cells:
                # Translate paragraphs (replace with English only)
                for para in cell.paragraphs:
                    if para.text.strip() and TextDetector.is_chinese(para.text):
                ...
                # Translate textboxes (English-only mode for cloned flowchart)
                textboxes = cell._element.findall('.//' + qn('w:txbxContent'))

                for textbox in textboxes:
                    # Translate all text elements to English
    ...
    def translate_textboxes_in_doc(self, doc):
        """Translate all textboxes in document (bilingual mode: keep Chinese, add English below)"""
        ...
        for textbox in textboxes:
            ...
            # Collect all Chinese text from paragraphs
            paragraphs = textbox.findall('.//' + qn('w:p'))
            all_chinese_texts = []

            for p in paragraphs:
                text_elems = p.findall('.//' + qn('w:t'))
                para_text = ''.join(t.text for t in text_elems if t.text)

                if para_text.strip() and TextDetector.is_chinese(para_text):
                    all_chinese_texts.append(para_text.strip())
                    # Adjust formatting for original Chinese text
                    self._adjust_textbox_formatting(textbox)

            # Translate and add English paragraph at the end
            if all_chinese_texts:
                merged_chinese = ' '.join(all_chinese_texts)
                translated_text = self.engine.translate_text(merged_chinese)
                if translated_text:
                    self.add_english_paragraph_to_textbox(textbox, translated_text, font_size_halfpts=11)
...
def main():
    ...
    # test_file = Path(r"I:\Multilingual Document Translation\W-QA-A070 ver02 原子吸收光譜儀(280FS AA)操作指導書.docx")
    ...
    # test_file = Path(r"I:\Multilingual Document Translation\PD-20 ver03 非測試介面先期產品流程管理程序_去英.docx")
    # test_file = Path(r"I:\Multilingual Document Translation\PD-19 ver02 潛在失效模式效應分析作業程序(中英版本)_去英.docx")
    
    output_file = Path("out_bilingual_refactored6.docx")
